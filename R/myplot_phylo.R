...
myplot.phylo <- function (x, type = "phylogram", use.edge.length = TRUE, node.pos = NULL, 
                          ...
                          align.tip.label = FALSE, ...) 
{
  ...
  .nodeDepth <- function(Ntip, 
    ...
    .C(node_depth, 
       ...
       as.integer(edge[, 2]), 
       as.integer(Nedge), 
       double(Ntip + Nnode), as.integer(node.depth))[[6]]
  
  .nodeDepthEdgelength <- function(Ntip, Nnode, edge, Nedge, edge.length) 
    
    .C(node_depth_edgelength, as.integer(Ntip), as.integer(Nnode), as.integer(edge[, 1]), 
       as.integer(edge[,  2]), as.integer(Nedge), as.double(edge.length), double(Ntip + Nnode))[[7]]
